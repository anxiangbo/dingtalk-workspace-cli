...
MCP tool: `list_open_dev_app_versions`（`--cursor`→`cursor`，`--page-size`→`pageSize`）。响应使用 `items` / `nextCursor` / `hasMore`。
...
MCP tool: `publish_open_dev_app_version`，CLI 强制 `precheckOnly=true`，仅返回审批要求和候选审批人，**不会实际发布**。
...
MCP tool: `publish_open_dev_app_version`，CLI 设 `precheckOnly=false`。
