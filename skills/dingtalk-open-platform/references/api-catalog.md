# 开放平台 API 文档目录（全量有效链接）

开放平台官方文档的**全量有效链接目录**，供本 skill 在发现阶段离线检索接口文档。

- URL 模板：`https://open.dingtalk.com/document/development/<slug>`（`<slug>` 来自真实爬取页面）。
- 有效性：经内容长度校验（真实页 ≥50KB，空壳 <50KB）后收录；统计见下方生成区块首行。
- 归类：按 8 能力域（doc_page_group）+ 接口 namespace / CSV 功能分类；无法归类入「其它 / 未归类」。
- 与本 skill [llm.md](../llm.md) **同源同步**，由 `scripts/build_llm_md.py` 生成，勿手改区块内容。
- 用法：按能力域 → 功能子类定位 `- [中文标题](URL): slug`，拿到 URL 后浏览文档，再用 `dws api` / `dws devapp` 调用（见 [api-invocation.md](api-invocation.md)）。
