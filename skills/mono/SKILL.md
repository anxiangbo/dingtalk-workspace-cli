...
| `pat`             | PAT 行为授权：授予 Agent 操作 scope、配置授权时是否允许打开浏览器               | [pat.md](./references/products/pat.md)                         |
...
用户提到"PAT/行为授权/授权 scope/永久授权/会话授权/授权时打开浏览器" → `pat`
...
| `pat` | `chmod` | 批量修改 Agent 行为授权 scope，可能扩大可执行权限 |
