...
用户说“导入 Excel”“把 xlsx 变成在线表格”“上传本地表格并在线编辑”时，使用 `dws sheet import`。它只新建在线电子表格，不覆盖已有表格。
...
| 命令 | 参数 | 说明 |
|------|------|------|
| `sheet import` | `--file` | 本地 xlsx/xls 文件；也可作为第一个位置参数 |
| `sheet import` | `--folder-token` | 目标文件夹 ID 或 URL，与 `--workspace` 至少传一个 |
| `sheet import` | `--workspace` | 目标知识库 ID 或 URL，与 `--folder-token` 至少传一个 |
| `sheet import` | `--name`, `-n` | 导入后的名称；默认取文件名并去掉扩展名 |
...
检测到 `timed_out:true` 后，只执行 `next_command` 续查。不要重新执行 `sheet import`。

## 边界

- 本命令接受的是本地文件路径；它把 xlsx/xls 转换成新的 axls 在线表格
- 已经存在于钉盘或文档中的 xlsx 节点不能直接传给工作表/单元格命令；先用 `doc download` 下载，需要在线编辑时再执行 `sheet import`
